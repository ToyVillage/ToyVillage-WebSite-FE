...
import { getAnimals, AnimalItem, AnimalListResponse } from '@/lib/api/animals';
...
const GROUPS: { key: keyof AnimalListResponse; label: string }[] = [
...
function AnimalCard({ animal, onClick }: { animal: AnimalItem; onClick: () => void }) {
  return (
    <div className="border rounded-xl p-4 shadow-sm cursor-pointer hover:shadow-md transition-shadow" onClick={onClick}>
      <div className="relative aspect-video bg-gray-200 rounded-lg mb-4 overflow-hidden">
        ...
        {animal.popular_animal && (
          <span className="absolute top-2 right-2 bg-main-forest-green text-white text-xs px-2 py-1 rounded-full">인기</span>
        )}
      </div>
...
export default function AdminAnimalsPage() {
  ...
  const animalData: AnimalListResponse = data ?? { popular_animal: [], mammals: [], reptiles: [], fish: [], birds: [] };

  const visibleGroups = GROUPS.filter(({ key }) => animalData[key].length > 0);

  return (
    <>
      <AdminSubHeader imageSrc={capybara} title="TOY VILLAGE" subtitle="동물 소개" />
      <div className="flex items-start">
        <aside className="sticky top-25 h-[calc(100vh-100px)] w-60 shrink-0 p-6 overflow-y-auto">
          <div className="mb-6">
            <AddButton href="/admin/animals/add" />
          </div>
            ...
            <section key={key} id={key} className="mb-16 scroll-mt-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {animalData[key].map((animal, i) => (
                  <AnimalCard key={`${animal.animal_kind}-${i}`} animal={animal} onClick={() => router.push(`/admin/animals/${animal.id}/edit`)} />
                ))}
              </div>
            </section>
          ))}
