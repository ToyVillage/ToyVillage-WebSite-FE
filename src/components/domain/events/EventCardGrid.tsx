          ...
          <li key={item.eventId}>
            {isAdmin ? (
              ...
              >
                <div className="relative w-full h-55 rounded-xl overflow-hidden mb-2">
                  {imageUrl != null ? (
                    <Image src={imageUrl} alt={item.eventName ?? ''} fill className="object-cover group-hover:scale-105 transition-transform duration-300" unoptimized />
                  ) : (
                    <div className="w-full h-full bg-gray-200" />
                  )}
                </div>
                ...
                <span className="flex items-center gap-1 text-body-3 text-gray-400 group-hover:text-black">
